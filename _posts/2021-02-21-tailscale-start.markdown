...
I've been following Tailscale (Brad, Christine, Avery, Dave, David and the gang) on Twitter, and this weekend I finally found some time to try it out!

Unfortunately, this post was a little underwhelming.....  
...
Before I start, thanks to [Kaden Barlow's](https://www.kadenbarlow.dev/blog/tailscale/) help on getting Tailscale to run in a Docker container. I feel like `systemd` is my nemesis, and I was *not* looking forward to messing with it, but Kaden made sure it played out of the box.

## What's Tailscale?

Tailscale is a zero-config mesh VPN based on Wireguard that 'just works'. It runs on desktops, laptops (Windows, MacOS, Linux), mobiles (IOS, Android), plus a bunch of other Unix-y or BSD-ish OSes and different platforms or architectures (pfSense, Synology, Ubiquity).

It promises to abstract away your network woes, punch holes through NATs, choose a geographically sensible relay node, provide DNS, enforce Access Control Lists and monitor services automatically.

In the end, you just get a **static** IP you can use to refer to other devices on your 'network'....and that's pretty much it!
...
I wanted to set up a Tailscale pod which would act as a relay, and advertise the K8S subnet. These subnet routes allow connecting machines to our Tailscale network without having Tailscale itself installed.
...
and built the image with `docker build . -t tpaslocal/tailscale`.

The entrypoint script creates a [Kubernetes configmap](https://kubernetes.io/docs/concepts/configuration/configmap/) and boots up Tailscale with a couple of arguments. We define how the 'machine' will be displayed as (`ubuntu-k8s` in our case), an Authentication key so that it can join our own network, as well as the aforementioned subnet routes that will be advertised.
...
<img src="/images/tailscale-subnet-review.png" style='height: 100%; width: 100%; object-fit: contain'/>
